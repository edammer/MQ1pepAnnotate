...
rootdir="~/SydneyRAW/total/"    #Contains RAW files and MaxQuant ./txt/ subfolder in it. The files proteinGroups.txt , msms.txt , and evidence.txt must be present
...
## Step 2) Build PDF of PSM annotations for each row in table compiled in previous step.

start_time <- Sys.time()

pdf(file="Annotated MS2 spectra of one-peptide protein identifications (Input Sample 31 RAW data fileSearch).PDF",width=11,height=8.5)
par(oma=c(0,1.5,3,2))
PSM<-list()  #thisMSMS<-list()
for (tableEntry in 25:27) {  #1:nrow(masterOHWtable)) {
	cat(paste0("Working on table entry (row) #",tableEntry,"...\n"))
	filenames.wd<-list.files(rootdir)
#	rawfile=paste0(rootdir, "/", filenames.wd[which(grepl(paste0(masterOHWtable[tableEntry,"Raw file"], "\\.[Rr][Aa][Ww]"),filenames.wd))])  #** with path
	rawfile=filenames.wd[which(grepl(paste0(masterOHWtable[tableEntry,"Raw file"], "\\.[Rr][Aa][Ww]"),filenames.wd))]
	thisScan=masterOHWtable[tableEntry,"MS/MS scan number"]
	thisPepSeq=gsub("_", "", masterOHWtable[tableEntry,"Modified sequence"])
#	thisPepSeq="(Acetyl (Protein N-term))TTQ(Deamidation (NQ))GGLVAN(Deamidation (NQ))RGR" #** example
	pepSeq.forPlotTitle=gsub("\\(Oxidation \\(M\\)\\)","*",
		gsub("\\(Deamidation \\(NQ\\)\\)","^",
		gsub("\\(Acetyl \\(Protein N-term\\)\\)","(Ac)",thisPepSeq) ))
	primaryPepSeq=masterOHWtable[tableEntry,"Sequence"]
	...
	PSM[[tableEntry]] <- rawrr::readSpectrum(rawfile=rawfile, scan=thisScan) |>
