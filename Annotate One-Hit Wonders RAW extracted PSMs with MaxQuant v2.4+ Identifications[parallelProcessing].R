...
#Inputs:
# Path to Thermo RAW files searched by MaxQuant v2.4+ to obtain the search result text files.
#  At and after v2.4, MaxQuant's step 1 outputs will have PTM annotations of (ox), (ac), and (de) for Met oxidation, N-terminal protein acetylation, and deamidation of Asn and Gln,
#  mass shifts for which the 2023 version of this tool will fail to annotate correctly in step 2.
...
#Outputs:
# Step 1:  Table of "one-hit wonder" single peptide (possibly multi-PSM) identifications of protein groups (readable by this script as well)
# Step 2:  PDF of annotated extracted MS/MS PSMs (the top, best scoring, for each identification in the above output table)
...
#rootdir="d:/tmp/HEK Files/"    #Contains RAW files and MaxQuant ./txt/ subfolder in it. The files proteinGroups.txt , msms.txt , and evidence.txt must be present
#rootdir="d:/tmp/HEK Lysate/"
#rootdir="d:/tmp/BV2lysate/"
rootdir="d:/tmp/BV2/"
...
write.table(masterOHWtable,file="Master One-peptide Protein Identification MS2 Spectra Annotations (via protViz)-TOTAL_all_RAW_data.txt",sep="\t",row.names=FALSE)


## Step 2) Build PDFs of PSM annotations for each row in table compiled in previous step. [ PARALLEL, IN CHUNKS ]

library(doParallel)
clusterLocal <- makeCluster(c(rep("localhost",31)),type="SOCK")
...
writePDFforChunk.df <- function(chunk.df) {
	require("protViz")
	require("rawrr")
	pdf(file=paste0("Annotated MS2 spectra of one-peptide protein identifications (Input Sample(s) RAW data fileSearch)[",chunk.df$'Raw file'[1],"].pdf"),width=11,height=8.5)
	par(oma=c(0,1.5,3,2))
	for (tableEntry in 1:nrow(chunk.df)) {
		...
		pepSeq.forPlotTitle=gsub("\\(ox\\)","*",
			gsub("\\(de\\)","^",
			gsub("\\(ac\\)","(Ac)",thisPepSeq) ))
		primaryPepSeq=chunk.df[tableEntry,"Sequence"]
		varMassShift.vec<-rep(0.0, nchar(primaryPepSeq))
		
		if (unlist(regexpr('\\(ac\\)', thisPepSeq))[1] == 1) {
		  varMassShift.vec[1]=varMassShift.vec[1]+42.010565				#6-digit monoisotopic precision from http://www.unimod.org/
		  thisPepSeq=gsub('\\(ac\\)', '', thisPepSeq)
		}
		
		
		#Handle M(Oxidation) residue mass shifts.
		if (unlist(regexpr('\\(ox\\)', thisPepSeq))[1] > 0) {
		  oxPTMcount=nrow(stringr::str_locate_all(thisPepSeq, '\\(ox\\)')[[1]])
		  for (i in 1:oxPTMcount) {
		    thisPosition=unlist(regexpr('\\(ox\\)', 
			gsub("\\(de\\)","",thisPepSeq)))[1] -1  #remove other mod annotations which will violate single letter residue notation assumption while counting character positions.
		    varMassShift.vec[thisPosition] <- varMassShift.vec[thisPosition] +15.994915
		    thisPepSeq=sub('\\(ox\\)', '', thisPepSeq)     #remove first instance match only, so that for loop can continue to find positions of later matches within single letter residue notation
		  }
		}
		
		#Handle NQ(Deamidation residue mass shifts.
		if (unlist(regexpr('\\(de\\)', thisPepSeq))[1] > 0) {
		  deamPTMcount=nrow(stringr::str_locate_all(thisPepSeq, '\\(de\\)')[[1]])
		  for (i in 1:deamPTMcount) {
		    thisPosition=unlist(regexpr('\\(de\\)', 
		        gsub("\\(ox\\)","",thisPepSeq)))[1] -1     #remove other mod annotations which will violate single letter residue notation assumption while counting character positions.
		    varMassShift.vec[thisPosition] <- varMassShift.vec[thisPosition] +0.984016
		    thisPepSeq=sub('\\(de\\)', '', thisPepSeq)  #remove first instance match only, so that for loop can continue to find positions of later matches within single letter residue notation
		  }
		}
		
		#Handle C-carbamidomethylation (static modification of Cysteine).
		if (unlist(regexpr('C', thisPepSeq))[1] > 0) {
		  carbamidoMethCount=nrow(stringr::str_locate_all(thisPepSeq, 'C')[[1]])
		  for (i in 1:carbamidoMethCount) {
		    thisPosition=unlist(regexpr('C', thisPepSeq))[1]
		    varMassShift.vec[thisPosition] <- varMassShift.vec[thisPosition] +57.021464
		  }
		}
